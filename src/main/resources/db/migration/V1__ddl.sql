...
create table participant (id int8 not null, score float4, competition_id int8, user_id int8, primary key (id));
...
alter table option add constraint option_constraint_task unique (option, task_id);
alter table participant add constraint UK_67f133k3yik28o7lnlsqry5fe unique (user_id, competition_id);
...
alter table participant_answer add constraint FKha22j4447ot7etlcjek1l4j1y foreign key (participant_id) references participant ON DELETE CASCADE ;
